...
type TypeCategory = 'root' | 'objects' | 'inputs' | 'enums' | 'scalars' | 'interfaces' | 'unions';
...
export function DocumentationView({ schema }: DocumentationViewProps) {
  ...
  const organizedTypes = useMemo<Record<TypeCategory, GraphQLNamedType[]>>(() => {
  ...
  const searchMatches = useMemo(() => {
    ...
    const addMatch = (typeName: string, match: SearchMatch) => {
      ...
      if (!existing.some((item) => item.kind === match.kind && item.label === match.label)) {
        existing.push(match);
      }
    ...
    const inspectType = (type: GraphQLNamedType) => {
      ...
      if (isObjectType(type) || isInterfaceType(type)) {
        for (const field of Object.values(type.getFields())) {
          const fieldText = [
            ...
            field.description,
            String(field.type),
            ...field.args.flatMap((arg) => [arg.name, arg.description, String(arg.type)]),
          ]
            .filter(Boolean)
            .join(' ')
            .toLowerCase();
          ...
          for (const arg of field.args) {
            if (
              [arg.name, arg.description, String(arg.type)].filter(Boolean).join(' ').toLowerCase().includes(lowerQuery)
            ) {
              addMatch(type.name, {
                kind: 'argument',
      ...
      if (isEnumType(type)) {
        for (const value of type.getValues()) {
          if ([value.name, value.description].filter(Boolean).join(' ').toLowerCase().includes(lowerQuery)) {
  ...
  const filteredTypes = useMemo(() => {
    ...
    (Object.keys(result) as TypeCategory[]).forEach((key) => {
      result[key] = organizedTypes[key].filter(
        (t) => t.name.toLowerCase().includes(lowerQuery) || searchMatches.has(t.name),
      );
    });
  ...
  const totalVisibleTypes = useMemo(
    () => Object.values(filteredTypes).reduce((count, types) => count + types.length, 0),
    [filteredTypes],
  );
        ...
        <div className="max-w-sm space-y-4 text-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-xs leading-5">
              Introspect the current endpoint to explore types, fields, arguments, enum values, interfaces, and unions.
            </p>
          </div>
        </div>
      </div>
  ...
  const isRootQuery = schema && selectedTypeObj && schema?.getQueryType()?.name === selectedTypeObj.name;
  const isRootMutation = schema && selectedTypeObj && schema?.getMutationType()?.name === selectedTypeObj.name;
  const isRootSubscription = schema && selectedTypeObj && schema?.getSubscriptionType()?.name === selectedTypeObj.name;

  let typeBadgeLabel = selectedTypeObj
    ? selectedTypeObj.constructor.name.replace('GraphQL', '').replace('Type', '')
    : '';
  ...
  return (
    <div className="bg-background flex h-full overflow-hidden rounded-md border">
      {/* Sidebar */}
      <div className="bg-muted/10 flex w-1/3 min-w-[250px] flex-col overflow-y-auto border-r">
        <div className="space-y-3 border-b p-3">
          <div className="flex items-center justify-between gap-2">
            <div>
              <div className="text-sm font-medium">Schema</div>
              <div className="text-muted-foreground text-xs">{totalVisibleTypes} visible types</div>
            </div>
            <Badge variant="outline" className="rounded-sm font-mono">
              {Object.keys(typeMap).filter((name) => !name.startsWith('__')).length}
            </Badge>
          </div>
        ...
        <ScrollArea className="flex-1">
          <div className="space-y-4 p-2">
            ...
            {(Object.entries(filteredTypes) as [TypeCategory, GraphQLNamedType[]][]).map(([category, types]) => {
              ...
              return (
                <div key={category}>
                  <div className="mb-2 flex items-center justify-between px-2">
                    ...
                    <span className="text-muted-foreground/70 font-mono text-[10px]">{types.length}</span>
                  </div>
                  <div className="space-y-0.5">
                    {types.map((type) => (
                      <Button
                        key={type.name}
                        variant={selectedType === type.name ? 'secondary' : 'ghost'}
                        size="sm"
                        className="h-7 w-full justify-start text-sm font-normal"
                        onClick={() => handleTypeClick(type.name)}
                      >
                        <TypeIcon type={type} className="mr-2 h-3.5 w-3.5 opacity-70" />
                        <span className="truncate">{type.name}</span>
                        {searchMatches.has(type.name) && (
                          <Badge variant="outline" className="ml-auto h-5 rounded-sm px-1.5 text-[10px]">
      ...
      <div className="bg-background flex min-w-0 flex-1 flex-col">
        {selectedTypeObj ? (
            ...
            <div className="space-y-6 p-6">
              <div className="bg-muted/10 space-y-3 rounded-md border p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <h1 className="text-primary font-mono text-xl font-semibold">{selectedTypeObj.name}</h1>
                </div>
                {selectedTypeObj.description && (
                  <p className="text-muted-foreground leading-relaxed">{selectedTypeObj.description}</p>
                )}
              ...
              {(isObjectType(selectedTypeObj) || isInterfaceType(selectedTypeObj)) && (
                  ...
                  <div className="space-y-2">
                    {Object.values(selectedTypeObj.getFields()).map((field) => {
                      ...
                      return (
                        <div
                          key={field.name}
                          className="group bg-card/40 data-[matched=true]:border-primary/50 data-[matched=true]:bg-primary/5 rounded-md border p-3"
                          data-matched={isMatched}
                        >
                          <div className="flex flex-wrap items-baseline gap-x-1 gap-y-1 font-mono text-sm">
                            <span className="font-bold text-sky-600 dark:text-sky-400">{field.name}</span>
                            {field.args.length > 0 && (
                              <span className="text-muted-foreground">
                                ...
                                {field.args.map((arg, i) => (
                                  <span key={arg.name}>
                                    {i > 0 && ', '}
                                    <span className="text-orange-600 dark:text-orange-400">{arg.name}</span>:{' '}
                            ...
                            {field.args.length > 0 && (
                              <Badge variant="outline" className="ml-2 h-5 rounded-sm text-[10px]">
                                {field.args.length} args
                              </Badge>
                            )}
                            {isMatched && (
                              <Badge variant="secondary" className="ml-2 h-5 rounded-sm text-[10px]">
                                Match
                              </Badge>
                            )}
                          ...
                          {field.description && (
                            <p className="text-muted-foreground mt-2 text-sm leading-5">{field.description}</p>
                          )}
                          {field.args.length > 0 && (
                            <div className="mt-3 space-y-1 border-t pt-2">
                              {field.args.map((arg) => (
                                <div key={arg.name} className="flex flex-wrap items-center gap-1.5 text-xs">
                                  <span className="font-mono text-orange-600 dark:text-orange-400">{arg.name}</span>
                  ...
                  <div className="space-y-4">
                    {Object.values(selectedTypeObj.getFields()).map((field) => (
                      <div key={field.name}>
                        ...
                        {field.description && (
                          <p className="text-muted-foreground border-muted mt-1 ml-4 border-l-2 pl-2 text-sm">
                            {field.description}
                          </p>
                        )}
                      </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selectedTypeObj.getValues().map((value) => (
                      <div key={value.name} className="bg-muted/30 rounded border p-2">
                        <div className="font-mono text-sm font-bold">{value.name}</div>
                        {value.description && (
                          <div className="text-muted-foreground mt-1 text-xs">{value.description}</div>
                        )}
          ...
          <div className="text-muted-foreground mx-auto my-auto max-w-md space-y-4 p-8 text-center">
            <Box className="mx-auto h-12 w-12 opacity-20" />
            <h3 className="text-lg font-medium">No Type Selected</h3>
            <p className="text-sm">
              Select a type from the sidebar to view its documentation, fields, and description.
            </p>
          </div>
        )}
      </div>
...
function TypeIcon({ type, className }: { type: GraphQLNamedType; className?: string }) {
