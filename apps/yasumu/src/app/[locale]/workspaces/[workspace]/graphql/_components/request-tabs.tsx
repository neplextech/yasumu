...
export function GraphqlRequestTabs({ tabs }: { tabs: RequestTab[] }) {
  ...
  return (
    <div className="flex items-center gap-2 select-none w-full max-w-[80vw]">
      ...
      >
        {tabs.map((tab, idx) => (
          <GraphqlTabItem tab={tab} key={tab.id} index={idx} />
        ))}
      </div>
      <EnvironmentSelector />
    </div>
...
function GraphqlTabItem({ tab, index }: { tab: RequestTab; index: number }) {
  const workspace = useActiveWorkspace();
  const graphql = workspace.graphql;
  const tabRef = useRef<HTMLDivElement>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['graphql-tab', tab.id],
    queryFn: async () => {
      const entity = await graphql?.get(tab.id);
      return entity?.data;
    },
    staleTime: 30_000,
