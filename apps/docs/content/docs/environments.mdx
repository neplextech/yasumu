...
<Callout type="warn" title="Secret Storage">
  Secret values are **never written to `.ysl` files**. The `value` field is
  always stored as an empty string. Actual secret values are stored securely and
  locally on your machine, separate from version control.
</Callout>
