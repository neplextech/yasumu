---
title: SMTP Server
description: Catch-all SMTP server for testing email functionality in
  development.
---
...
<Callout type="info" title="Email Event">
  The `onEmail` function is called every time a new email is received by the
  SMTP server. Use conditional checks to filter which emails your script
  processes.
</Callout>
...
export function onEmail(ctx: YasumuScriptContext, email: YasumuEmail) {
  ...
  const tokenMatch = email.html.match(/reset-password\?token=([^"&]+)/);
  if (tokenMatch) {
...
```typescript
export function onEmail(ctx: YasumuScriptContext, email: YasumuEmail) {
  if (!email.subject.toLowerCase().includes('verify your email')) return;
...
<Callout type="tip" title="Chaining with API Requests">
  Variables set via email scripts can be used immediately in your API requests
  using the `{{ VARIABLE_NAME }}` syntax. This enables automated testing of
