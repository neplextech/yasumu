...
<Callout type="tip" title="Scripting Language">
  Yasumu supports both `JavaScript` and `TypeScript` syntax for scripting
  out-of-the-box. The editor also provides type definitions and autocomplete for
...
<Callout type="info" title="Looking for e-mail scripting?">
  Please refer to the [E-mail Scripting](/docs/smtp-server#email-scripting)
  guide for detailed overviews on how to add email listener in Yasumu.
</Callout>
...
| `workspace` | `YasumuWorkspace`            | The current workspace object                     |
...
<Callout type="info" title="Environment Persistence">
  Variables set via `setVariable()` are automatically saved to the active
  environment after the script completes. This allows you to chain requests and
...
Yasumu offers a powerful API for programmatically awaiting incoming
emails within your scripts. This functionality is particularly
valuable for testing and development scenarios that require
verification of email delivery as part of an automated workflow.

```javascript
export async function onRequest(req: YasumuRequest) {
  // Wait for an email to arrive in the workspace's mailbox.
  // The predicate should return true for emails that match your criteria.
  // The optional timeout (in milliseconds) determines how long to wait.
  const email = await req.workspace.emails.awaitEmail(
    (email) => /* your predicate logic here */,
    /* timeout */ 30_000
  );
...
// It may also be added in the `onResponse` function to await an email after a request is made.
export async function onResponse(req: YasumuResponse) {
  const email = await req.workspace.emails.awaitEmail(
    (email) => /* your predicate logic here */,
    /* timeout */ 30_000
  );
...
For detailed examples and advanced features, please refer to the
dedicated [E-mail Scripting](/docs/smtp-server#email-scripting) guide.

## Console Output

Use `console.log()` in your scripts to output debug information. The
output appears inside the **Console** section on the status bar, which
is available at the bottom right side of the screen.
...
![Console Output](/view-console-logs.png)
...
<Callout type="warn" title="Permission System">
...
![Permission Prompt](/dangerous-script-asks-for-permission-prompt.png)
...
export async function onRequest(req) {
  ...
  const message = encoder.encode(`${req.method}:${req.url}:${timestamp}`);
...
<Callout type="info" title="CUID vs UUID">
  CUIDs are shorter, URL-safe, and designed to be monotonically increasing,
  making them ideal for database IDs and distributed systems.
</Callout>
...
<Callout type="info" title="Async Functions">
  Scripts support `async`/`await`. If you use async APIs like `crypto.subtle`,
  make sure to declare your function as `async` and `await` the results.
</Callout>
