...
<Callout type="tip" title="Global APIs">
  The `test`, `expect` and `describe` functions are available globally in your
  scripts. No imports needed.
</Callout>

<Callout type="warn" title="Test Context Required">
  The `test()` and `expect()` functions only work inside the `onTest()`
  function. Calling them in `onRequest()` or `onResponse()` will have no effect
  as they are silently ignored in non-test mode.
</Callout>
...
export function onRequest(req) {
  req.headers.set('Authorization', `Bearer ${req.env.getSecret('TOKEN')}`);
}
...
<Callout type="info" title="Tests Only Run in onTest">
  If you accidentally call `onTest()` inside `onRequest` or `onResponse`, it
  will only produce log output — no tests are actually executed. This is by
  design to ensure tests only run in test mode.
</Callout>
...
<Callout type="info" title="Test Execution">
  Tests run synchronously in the order they're defined. The `onTest` function is
  called after `onResponse` completes.
</Callout>
