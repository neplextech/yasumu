...
description: Execute and manage your Yasumu workspaces from the terminal using
  the CLI.
---
...
<Callout type="info" title="YSL File Execution">
  The CLI reads and executes `.ysl` (Yasumu Schema Language) files directly from
  your workspace, making it perfect for headless environments and automated
  testing.
</Callout>
...
<Callout type="warn" title="Security">
  Never commit actual secret values to your repository. Use the `YASUMU_ENV_`
  prefix pattern to inject secrets at runtime.
</Callout>
...
<Callout type="info" title="Compatibility Note">
  Some advanced TypeScript features may not be fully supported. For complex
  scripts, consider using plain JavaScript.
</Callout>
