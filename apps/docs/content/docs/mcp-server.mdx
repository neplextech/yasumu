...
4. Run the request script `onRequest` handler when a script is
   present.
...
9. Return the request, response, lifecycle summaries, and test
   results.
...
`rest_execute` returns a text MCP result containing JSON. The shape
is:
...
Check the returned `error` field from `rest_execute`. Common causes
are:
