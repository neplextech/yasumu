...
`createSearchParamsCache` requires calling `parse()` at the page level
before calling `get()` in nested components. Forgetting `parse()`
causes `get()` to return undefined or throw.
