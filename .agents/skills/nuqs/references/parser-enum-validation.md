...
tags:
  parser, parseAsStringEnum, parseAsStringLiteral, validation,
  security
---

## Use Enum Parsers for Constrained Values

When state should only accept specific values (like status, sort
direction, or view mode), use enum or literal parsers. This prevents
invalid values from URL tampering and provides type safety.
...
const [status, setStatus] = useQueryState(
  'status',
  parseAsStringEnum<Status>(Object.values(Status)).withDefault(
    Status.Active,
  ),
);
```
