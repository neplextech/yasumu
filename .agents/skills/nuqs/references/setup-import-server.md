...
impact: CRITICAL
impactDescription:
  prevents RSC-to-client boundary contamination errors
tags:
  setup, nuqs/server, imports, createSearchParamsCache,
  server-components
---

## Import Server Utilities from nuqs/server

Server-side utilities like `createSearchParamsCache` must be imported
from `nuqs/server`, not `nuqs`. The main `nuqs` export includes the
`'use client'` directive which contaminates Server Components.
