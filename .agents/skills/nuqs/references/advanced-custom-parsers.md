...
When built-in parsers don't fit your needs, create custom parsers with
`createParser`. Define `parse`, `serialize`, and optionally `eq` for
equality checking.
...
export default function SortableTable() {
  ...
  const handleSort = (id: string) => {
    ...
    setSortRaw(
      `${id}:${sort.id === id && !sort.desc ? 'desc' : 'asc'}`,
    );
  };
}
```
