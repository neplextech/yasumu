...
When multiple URL parameters are logically related (like coordinates,
date ranges, or filters), use `useQueryStates` for atomic updates.
Multiple `useQueryState` calls update the URL independently, causing
intermediate states.
