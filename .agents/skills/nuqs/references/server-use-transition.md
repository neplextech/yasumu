...
When using `shallow: false`, integrate React's `useTransition` to
track when the server is fetching new data. This enables loading
indicators during URL-triggered server updates.
...
Reference:
[nuqs useTransition Integration](https://nuqs.dev/docs/options)
