...
tags:
  parser, parseAsTimestamp, parseAsIsoDateTime, parseAsIsoDate, dates
---

## Select Appropriate Date Parser

nuqs provides three date parsers with different URL formats and
precision. Choose based on your requirements for time precision and
URL readability.
...
export default function DateRangePicker() {
  ...
  const [lastModified, setLastModified] = useQueryState(
    'modified',
    parseAsTimestamp,
  );
