...
When multiple components use the same URL parameters, define parsers
in a shared file. This prevents mismatches where one component uses
`parseAsInteger` and another uses `parseAsString` for the same
parameter.
...
export const searchParams = {
  ...
  sort: parseAsStringLiteral(['asc', 'desc'] as const).withDefault(
    'asc',
  ),
};
