...
export function createParamDecorator(factory: CustomParamFactory): (data?: any) => ParameterDecorator {
  return (data?: any) => (target: Object, propertyKey: string | symbol | undefined, parameterIndex: number) => {
    if (!propertyKey) return;
    const params = Reflect.getMetadata(RESOLVER_METADATA.PARAMS, target.constructor) || {};
    ...
    Reflect.defineMetadata(RESOLVER_METADATA.PARAMS, params, target.constructor);
  };
}
