...
import type { EmailProvider, EntityRepository, FileResolver, PermissionProvider } from './ports.js';
  ...
  return async <K extends RuntimeHostMethod>(
    ...
    switch (method) {
      case 'entity.get': {
        ...
        const entity = await dependencies.entities.get(context.workspace.id, request.id);
        return (entity?.kind === request.kind ? toScriptEntity(entity) : null) as RuntimeHostCalls[K]['output'];
      }
      case 'entity.list': {
        const request = value as RuntimeHostCalls['entity.list']['input'];
        const entities = await dependencies.entities.list(context.workspace.id, request.kind);
      ...
      case 'entity.execute': {
        ...
        const target = await dependencies.entities.get(context.workspace.id, request.id);
        if (!target) {
          throw hostError(YasumuErrorCodes.EntityNotFound, `Entity not found: ${request.id}`, context, request.id);
        }
        ...
        return (await dependencies.execute(request, combined)) as RuntimeHostCalls[K]['output'];
      }
      case 'email.list': {
        if (!dependencies.email) {
          throw hostError(YasumuErrorCodes.PermissionDenied, 'Email capability is unavailable', context);
        }
        const request = value as RuntimeHostCalls['email.list']['input'];
        const emails = await dependencies.email.list(context.workspace.id, request, combined);
      ...
      case 'file.resolve': {
        if (!dependencies.files) {
          throw hostError(YasumuErrorCodes.FileAccessDenied, 'File capability is unavailable', context);
        }
        const request = value as RuntimeHostCalls['file.resolve']['input'];
        return (await dependencies.files.resolve(context.workspace, request.path)) as RuntimeHostCalls[K]['output'];
      }
      case 'file.open': {
        if (!dependencies.files) {
          throw hostError(YasumuErrorCodes.FileAccessDenied, 'File capability is unavailable', context);
        ...
        const opened = await dependencies.files.open(context.workspace, request.reference, combined);
        return {
          file: opened.file,
      ...
      case 'permission.request': {
        ...
        const granted = dependencies.permissions ? await dependencies.permissions.request(request, combined) : false;
