...
export class GraphqlModule {
  ...
  public async create(data: GraphqlEntityCreateOptions): Promise<GraphqlEntity> {
    const result = await this.workspace.manager.yasumu.rpc.graphql.create.$mutate({
      parameters: [data],
    });
  ...
  public async createBulk(items: GraphqlEntityCreateOptions[]): Promise<GraphqlEntity[]> {
    const results = await this.workspace.manager.yasumu.rpc.graphql.createBulk.$mutate({
      parameters: [{ items }],
    });
  ...
  public async update(id: string, data: Partial<GraphqlEntityUpdateOptions>): Promise<GraphqlEntityData> {
    const result = await this.workspace.manager.yasumu.rpc.graphql.update.$mutate({
      parameters: [id, data],
    });
  ...
  public async listTree(): Promise<GraphqlTreeItem[]> {
    const data = await this.workspace.manager.yasumu.rpc.graphql.listTree.$query({
      parameters: [],
    });
  ...
  public async executeScript(entityId: string, script: YasumuEmbeddedScript, context: GraphqlScriptContext) {
    const result = await this.workspace.manager.yasumu.rpc.graphql.executeScript.$mutate({
      parameters: [
        {
          entityId,
          script,
  ...
  public async executeTest(entityId: string, script: YasumuEmbeddedScript, context: GraphqlScriptContext) {
    const result = await this.workspace.manager.yasumu.rpc.graphql.executeScript.$mutate({
      parameters: [
        {
          entityId,
