...
import { getGuiEchoServerPort, GuiFetchTransport, rewriteGuiEchoUrl, setGuiEchoServerPort } from './echo-transport.ts';
import { GuiExecutionEventSink } from './event-sink.ts';
import { GuiFileResolver, InMemoryGuiFileHandleStore } from './file-resolver.ts';
import { type GuiConfirmationDialog, GuiPermissionProvider } from './permission-provider.ts';
...
Deno.test('GUI file resolver supports serializable references and blocks traversal and symlink escapes', async () => {
  ...
  try {
    ...
    await Promise.all([mkdir(join(root, 'fixtures'), { recursive: true }), mkdir(outside, { recursive: true })]);
    await Promise.all([
    ...
    const workspaceFile = await resolver.open(workspace, {
      id: 'inside',
      name: 'inside.txt',
    ...
    const cliFile = await new NodeWorkspaceFileResolver().open(cliWorkspace, {
      id: 'inside',
      name: 'inside.txt',
      source: { type: 'workspace-path', path: 'fixtures/inside.txt' },
    });
    ...
    await assert.rejects(() => resolver.resolve(workspace, '../outside/secret.txt'), /escapes the workspace root/);
    await assert.rejects(() => resolver.resolve(workspace, 'escape/secret.txt'), /escapes the workspace root/);
  } finally {
...
Deno.test('GUI permission and event adapters keep native decisions blocking and events observational', async () => {
  ...
  await assert.rejects(
    () => permissions.request({ capability: 'network', executionId: 'execution' }, aborted.signal),
    /Stopped/,
  );
...
Deno.test('GUI assembly executes through fake runtime, Drizzle ports, events, transport, and history', async () => {
  ...
  try {
    ...
    assert.deepEqual(result.response?.body.kind === 'json' ? result.response.body.value : null, { echoed: true });
    assert.deepEqual(urls, ['http://127.0.0.1:32100/api/echo?source=gui']);
    assert.deepEqual(
      events
        .filter((event) => event.type === 'execution-started' || event.type === 'execution-completed')
...
Deno.test('desktop and in-memory CLI-style assemblies produce the same normalized execution result', async () => {
  ...
  try {
    ...
    assert.deepEqual(normalizeExecution(guiResult), normalizeExecution(cliResult));
  } finally {
    database.$client.close();
  }
});
...
class FakeRuntime implements YasumuScriptRuntime {
  ...
  public createSession(_input: CreateRuntimeSessionInput): Promise<YasumuRuntimeSession> {
    return Promise.resolve(new FakeRuntimeSession());
  }
}

class FakeRuntimeSession implements YasumuRuntimeSession {
  public invokeHook(invocation: ScriptHookInvocation, _options?: InvokeHookOptions): Promise<ScriptHookResult> {
    return Promise.resolve({
...
class RecordingEmailRuntime extends FakeRuntime {
  ...
  public override createSession(input: CreateRuntimeSessionInput): Promise<YasumuRuntimeSession> {
    return Promise.resolve({
      invokeHook: async (invocation, options) => {
        this.invocations.push(invocation);
        const signal = options?.signal ?? new AbortController().signal;
        const entities = await input.hostCall('entity.list', { kind: 'rest' }, signal);
...
function executionEvent(type: 'execution-started'): Extract<ExecutionEvent, { type: 'execution-started' }> {
  return {
    type,
    executionId: 'execution',
    workspaceId: 'workspace',
...
>(result: T) {
  const { startedAt: _startedAt, completedAt: _completedAt, durationMs: _durationMs, ...stable } = result;
  return stable;
}
