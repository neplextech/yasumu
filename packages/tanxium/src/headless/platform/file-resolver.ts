...
export interface GuiFileHandleStore {
  get(handleId: string, signal?: AbortSignal): Promise<SerializableGuiFile | null>;
}
...
export class InMemoryGuiFileHandleStore implements GuiFileHandleStore {
  ...
  public register(handleId: string, file: SerializableGuiFile): YasumuFileReference {
  ...
  public async get(handleId: string, signal?: AbortSignal): Promise<SerializableGuiFile | null> {
...
export class GuiFileResolver implements FileResolver {
  public constructor(private readonly handles: GuiFileHandleStore = new InMemoryGuiFileHandleStore()) {}

  public async resolve(workspace: YasumuWorkspace, path: string): Promise<ResolvedScriptFile> {
    const resolved = await resolveWorkspaceFile(workspace, path);
    const information = await stat(resolved.absolutePath);
    if (!information.isFile()) {
      throw fileError(YasumuErrorCodes.FileNotFound, `Workspace file is not a regular file: ${path}`, workspace, path);
    }
    return {
  ...
  public async open(
    ...
    switch (reference.source.type) {
      ...
      case 'workspace-path': {
        ...
        const bytes = await readFile(file.resolvedPath!, signal ? { signal } : undefined);
        signal?.throwIfAborted();
...
async function resolveWorkspaceFile(
  ...
  if (!workspace.root) {
    throw fileError(YasumuErrorCodes.FileAccessDenied, 'This workspace does not expose a filesystem root', workspace);
  }
  ...
  try {
    ...
    const [root, candidate] = await Promise.all([realpath(sourceRoot), realpath(resolve(sourceRoot, requestedPath))]);
...
function openSerializable(reference: YasumuFileReference, input: SerializableGuiFile): FileOpenResult {
  const file = cloneSerializableFile(input);
  const mimeType = reference.mimeType ?? file.mimeType ?? inferMimeType(file.name);
  return {
    file: {
      ...reference,
      name: reference.name || file.name,
...
function cloneSerializableFile(file: SerializableGuiFile): SerializableGuiFile {
  ...
  if (!file.bytes.every((byte) => Number.isInteger(byte) && byte >= 0 && byte <= 255)) {
    throw new TypeError('GUI file bytes must contain integers between 0 and 255');
  }
...
function fileError(
  code: typeof YasumuErrorCodes.FileNotFound | typeof YasumuErrorCodes.FileAccessDenied,
  message: string,
  workspace: YasumuWorkspace,
  file?: string,
  cause?: unknown,
