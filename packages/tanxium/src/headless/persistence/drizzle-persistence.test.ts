...
import { type ExecutableEntity, WorkspaceCrudService, type YasumuWorkspace } from '@yasumu/headless';
...
Deno.test('workspace adapter maps complete aggregates without claiming source baseline ownership', async () => {
  ...
  try {
    ...
    const revisions = database.select().from(sourceRevisions).where(eq(sourceRevisions.workspaceId, 'workspace')).all();
...
Deno.test('entity adapter enforces stable IDs, references, deterministic listing, and CRUD', async () => {
  ...
  try {
    ...
    await assert.rejects(() => persistence.entities.delete('workspace', 'rest-base'), /still referenced/);
    await assert.rejects(() => persistence.entities.create('workspace', graphqlEntity('rest-base')), /already exists/);
    ...
    const updated = await persistence.entities.update('workspace', 'rest-child', {
      ...child,
      name: 'Updated child',
      dependencies: [],
    });
    ...
    assert.equal(await persistence.entities.get('workspace', 'rest-base'), null);
  } finally {
    database.$client.close();
  }
});
...
Deno.test('shared workspace CRUD deletes omitted environment, group, and SMTP rows', async () => {
  ...
  try {
    ...
    assert.equal(database.select().from(environments).where(eq(environments.id, 'environment-a')).get(), undefined);
    assert.equal(database.select().from(entityGroups).where(eq(entityGroups.id, 'group-child')).get(), undefined);
    assert.equal(database.select().from(smtp).where(eq(smtp.workspaceId, 'workspace')).get(), undefined);
    assert.equal(
      database.select().from(workspaces).where(eq(workspaces.id, 'workspace')).get()?.activeEnvironmentId,
      null,
    );
  } finally {
    database.$client.close();
...
Deno.test('environment and email adapters use deterministic workspace-scoped persistence', async () => {
  ...
  try {
    ...
    const first = await persistence.email.next(
      'workspace',
      {
        since: 10,
        timeoutMs: 0,
    ...
    const second = await persistence.email.next(
      'workspace',
      {
        since: 10,
        cursor: first.cursor,
    ...
    const done = await persistence.email.next(
      'workspace',
      {
        since: 10,
    ...
    environments: [
      {
        ...
        origin: ysl('yasumu/environment/environment-a.ysl', 'environment-revision'),
      },
    ],
    smtp: {
      id: 'smtp',
...
function restEntity(id: string, dependencies: string[] = []): ExecutableEntity {
  return {
    ...
    scripts: {
      ...
      test: id === 'rest-b' ? { id: `${id}-test`, code: 'export function onTest() {}' } : undefined,
    },
    dependencies,
    metadata: {},
