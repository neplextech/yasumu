...
export class DrizzleWorkspaceRepository implements WorkspaceRepository {
  ...
  public async get(workspaceId: string): Promise<YasumuWorkspace | null> {
    const workspace = this.database.select().from(workspaces).where(
      eq(workspaces.id, workspaceId),
    ).get();
    if (!workspace) return null;

    const revisions = sourceMap(
      loadSourceRevisions(this.database, workspaceId),
    );
    const origin = mapOrigin(
      revisions.get(sourceKey("workspace", workspaceId)),
    );
    const groups = orderGroups(
      this.database
        ...
        .all()
        .map((row) =>
          mapGroup(row, revisions.get(sourceKey("entity-group", row.id)))
        ),
    );
    const workspaceEnvironments = this.database
      ...
      .map((row) =>
        mapEnvironment(row, revisions.get(sourceKey("environment", row.id)))
      );
    const smtpRow = this.database.select().from(smtp).where(
      eq(smtp.workspaceId, workspaceId),
    ).get();
    ...
    return {
      ...
      smtp: smtpRow
        ? mapSmtp(smtpRow, revisions.get(sourceKey("smtp", smtpRow.id)))
        : undefined,
  ...
  public async save(workspace: YasumuWorkspace): Promise<void> {
    ...
    this.database.transaction((transaction) => {
      ...
      const restIds = workspace.entities.filter((entity) =>
        entity.kind === "rest"
      ).map((entity) => entity.id);
      const graphqlIds = workspace.entities.filter((entity) =>
        entity.kind === "graphql"
      ).map((entity) => entity.id);
      const environmentIds = workspace.environments.map((environment) =>
        environment.id
      );
        ...
        .where(
          restIds.length
            ? and(
              ...
              notInArray(restEntities.id, restIds),
            )
            : eq(restEntities.workspaceId, workspace.id),
        )
        .run();
        ...
        .where(
          graphqlIds.length
            ? and(
              ...
              notInArray(graphqlEntities.id, graphqlIds),
            )
            : eq(graphqlEntities.workspaceId, workspace.id),
        )
        .run();
        ...
        .where(
          environmentIds.length
            ? and(
              ...
              notInArray(environments.id, environmentIds),
            )
            : eq(environments.workspaceId, workspace.id),
        )
        .run();
        ...
        .where(
          groupIds.length
            ? and(
              ...
              notInArray(entityGroups.id, groupIds),
            )
            : eq(entityGroups.workspaceId, workspace.id),
        )
        .run();
      if (!workspace.smtp) {
        transaction.delete(smtp).where(eq(smtp.workspaceId, workspace.id))
          .run();
      }
    });
  }
...
function assertAggregateOwnership(
  ...
  if (allEntityIds.length > 0) {
    const storedRest = database.select().from(restEntities).where(
      inArray(restEntities.id, allEntityIds),
    ).all();
    ...
    const conflict = [
      ...
      const incoming = workspace.entities.find((entity) =>
        entity.id === stored.id
      );
      return stored.workspaceId !== workspace.id ||
  ...
  if (workspace.smtp) {
    const existing = database.select().from(smtp).where(
      eq(smtp.id, workspace.smtp.id),
    ).get();
...
function ownershipError(workspaceId: string, kind: string, id: string): never {
  throw new YasumuError(
    ...
    `${kind} ID belongs to another workspace: ${id}`,
    {
      workspaceId,
    },
  );
...
function validateWorkspace(workspace: YasumuWorkspace): void {
  ...
  for (const group of workspace.groups) {
    if (
      ...
      (group.parentId && !groups.has(group.parentId))
    ) {
      throw new YasumuError(
        YasumuErrorCodes.InvalidReference,
        `Invalid group reference: ${group.id}`,
        {
          workspaceId: workspace.id,
        },
      );
  ...
  for (const entity of workspace.entities) {
    if (entity.workspaceId !== workspace.id) {
      throw new YasumuError(
        YasumuErrorCodes.InvalidReference,
        `Entity ${entity.id} belongs to another workspace`,
        {
          workspaceId: workspace.id,
          entityId: entity.id,
        },
    ...
    if (
      ...
      throw new YasumuError(
        YasumuErrorCodes.InvalidReference,
        `Invalid group for entity ${entity.id}`,
        {
          workspaceId: workspace.id,
          entityId: entity.id,
        },
    ...
    for (const dependency of entity.dependencies) {
      if (entities.get(dependency)?.kind !== entity.kind) {
        throw new YasumuError(
          YasumuErrorCodes.InvalidReference,
          `Invalid same-kind dependency: ${dependency}`,
          {
            workspaceId: workspace.id,
            entityId: entity.id,
          },
  ...
  if (
    ...
    !workspace.environments.some((environment) =>
      environment.id === workspace.activeEnvironmentId
    )
  ) {
    throw new YasumuError(
      YasumuErrorCodes.InvalidReference,
...
function orderGroups(groups: WorkspaceGroup[]): WorkspaceGroup[] {
  ...
  const visit = (group: WorkspaceGroup) => {
    ...
    if (current === "visiting") {
      throw new YasumuError(
        YasumuErrorCodes.InvalidReference,
        `Group cycle detected at ${group.id}`,
      );
    }
  ...
  for (
    const group of [...groups].sort((left, right) =>
