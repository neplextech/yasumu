...
export type GuiExecuteEntityInput = Omit<ExecuteEntityInput, 'workspaceId' | 'signal'>;
...
export class ExecutionService {
  ...
  public execute(workspaceId: string, input: GuiExecuteEntityInput): Promise<ExecutionResult> {
    return this.platform.execution.execute({ ...input, workspaceId });
  }

  public cancel(workspaceId: string, executionId: string, reason?: string): boolean {
  ...
  private trackActiveExecution(event: ExecutionEvent): void {
    if (event.type === 'execution-started') {
      const active = this.activeByWorkspace.get(event.workspaceId) ?? new Set<string>();
