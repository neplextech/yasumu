...
port.on('message', (message: unknown) => {
  ...
  switch (message.type) {
    ...
    case 'cancel': {
      ...
      rejectHostCalls(message.requestId, workerError('SCRIPT_CANCELLED', message.reason ?? 'Execution cancelled'));
      return;
    }
    case 'dispose':
      for (const [requestId, state] of activeExecutions) {
        ...
        state.controller.abort(state.cancelReason);
        rejectHostCalls(requestId, workerError('SCRIPT_SESSION_DISPOSED', state.cancelReason));
      }
      port.close();
  }
});
...
async function invoke(requestId: string, invocation: ScriptHookInvocation): Promise<void> {
  ...
  const state: WorkerExecutionState = {
    ...
    controller,
    environment,
    request: invocation.request ? requestFromSnapshot(invocation.request) : undefined,
    requestBackup: invocation.request ? requestFromSnapshot(invocation.request) : undefined,
    response: invocation.response ? responseFromSnapshot(invocation.response) : undefined,
    logs: [],
    diagnostics: [],
    tests: [],
    testRun: invocation.hook === 'onTest' ? { tests: [], pendingSuites: [] } : undefined,
  ...
  try {
    ...
    if (!failedExecutions.has(requestId)) {
      port.postMessage({ type: 'result', requestId, result } satisfies RuntimeOutboundMessage);
    }
  } catch (error) {
    ...
    rejectHostCalls(requestId, workerError('SCRIPT_EXECUTION_ENDED', 'The script execution has ended'));
  }
}

async function executeHook(state: WorkerExecutionState): Promise<ScriptHookResult> {
  if (state.invocation.hook === 'onTest' && state.invocation.execution.mode !== 'test') {
    throw workerError('SCRIPT_TEST_MODE_REQUIRED', 'onTest can only run during an explicit test execution');
  }

  const moduleUrl = registerScriptSource(state.invocation.source, state.invocation.workspace.root);
...
async function invokeRequestHook(state: WorkerExecutionState, hook: ScriptHook): Promise<void> {
  if (!state.request || !state.invocation.request) {
    throw workerError('SCRIPT_INVALID_INVOCATION', 'onRequest requires a request');
  }

  const legacyRequest = new LegacyRequest(state.request, state.invocation.request);
  ...
  const context: RequestHookContext = {
    ...
    setRequest(request) {
      if (!(request instanceof Request) || request.bodyUsed) {
        throw workerError('SCRIPT_INVALID_REQUEST', 'setRequest requires an unconsumed standard Request');
      }
  ...
  const value =
    hook.length >= 2 ? await hook(legacyRequest, null) : await hook(withLegacyRequestAliases(context, legacyRequest));
...
async function invokeResponseHook(state: WorkerExecutionState, hook: ScriptHook, isTest: boolean): Promise<void> {
  if (!state.request || !state.response || !state.invocation.request || !state.invocation.response) {
    throw workerError('SCRIPT_INVALID_INVOCATION', `${state.invocation.hook} requires a request and response`);
  }
  ...
  const context = isTest ? ({ ...responseContext, isTest: true } satisfies TestHookContext) : responseContext;
...
async function invokeEmailHook(state: WorkerExecutionState, hook: ScriptHook): Promise<void> {
...
async function createResult(state: WorkerExecutionState): Promise<ScriptHookResult> {
  return {
    request: state.request ? await snapshotPossiblyConsumedRequest(state) : undefined,
    mockResponse: state.mockResponse ? await snapshotResponse(state.mockResponse, Number.POSITIVE_INFINITY) : undefined,
...
async function snapshotPossiblyConsumedRequest(state: WorkerExecutionState) {
  ...
  if (!state.requestBackup) {
    throw workerError('SCRIPT_REQUEST_BODY_UNAVAILABLE', 'The consumed request body could not be restored');
  }
...
function installConsoleCapture(): void {
  ...
  for (const [method, level] of Object.entries(methods) as Array<
...
function installUnhandledErrorCapture(): void {
  const report = (error: unknown) => {
    ...
    rejectHostCalls(state.requestId, workerError('SCRIPT_UNHANDLED_ERROR', 'Unhandled script error'));
    port.postMessage({
      type: 'error',
