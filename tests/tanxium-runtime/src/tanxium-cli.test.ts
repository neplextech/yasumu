...
function run(
  source: string,
  options: { verbose?: boolean; sandbox?: boolean; noSandbox?: boolean } = {},
) {
...
function runFailureAsync(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill("SIGKILL");
      reject(
        new Error("Tanxium did not finish reporting the source-mapped failure"),
      );
    }, 60_000);
...
async function sendSmtpMessage(): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      socket.destroy();
      reject(
        new Error(`SMTP server did not finish the exchange:\n${transcript}`),
      );
    }, 5_000);
...
describe("tanxium CLI runtime semantics", () => {
  ...
  it("loads HTTP modules with external source maps", async () => {
    ...
    const server = createServer((request, response) => {
      const content = request.url === "/remote-entry.js"
        ? generatedSource
    ...
    try {
      const entrypoint =
        /* js */ `await import('http://127.0.0.1:${address.port}/remote-entry.js');`;
  ...
  it("forwards renderer events through the terminal host", () => {
    const output = run(
      /* js */ `Yasumu.postMessage({ type: 'runtime-event', value: 42 });`,
      { verbose: true },
    );
  ...
  it("imports an ESM package from the workspace node_modules directory", () => {
    ...
    writeFileSync(
      ...
      `export const value = 'resolved from node_modules';`,
    );
  ...
  it("loads CommonJS packages and serves SMTP over Node TCP", async () => {
    ...
    try {
      await new Promise<void>((resolveReady, rejectReady) => {
        const timeout = setTimeout(() => {
          rejectReady(
            new Error(`SMTP server did not start:\n${output}\n${errors}`),
          );
        }, 5_000);
        ...
        child.once("exit", (code) => {
          clearTimeout(timeout);
          rejectReady(
            new Error(`SMTP server exited early with code ${code}:\n${errors}`),
          );
        });
      });
  ...
  it("passes the shared headless runtime conformance suite", () => {
    const entrypoint = join(
      root,
      "tests/tanxium-runtime/fixtures/headless-runtime-conformance.ts",
    );
